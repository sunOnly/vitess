...
func (v Value) ToMySQL() ([]byte, error) {
	...
	case Decimal, Text, Blob, VarChar, VarBinary, Char, Bit, Enum, Set, Geometry, Binary, TypeJSON:
		l := len(v.val)
...
func (v Value) ToMySQLLen() (int, error) {
	...
	case Decimal, Text, Blob, VarChar, VarBinary, Char, Bit, Enum, Set, Geometry, TypeJSON:
		l := len(v.val)
		length = lenEncIntSize(uint64(l)) + l
	case Binary:
		length = len(v.val)
